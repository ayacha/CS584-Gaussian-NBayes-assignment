...
for(i in 1:m) {
  for(j in 1:n) {
    if(y[i,]==1){
      trainYpredictions1[i] = trainYpredictions1[i] * ( (alpha1[j])^(x[i,j]) * (1-alpha1[j])^(1-x[i,j]));
    }
  }
}
...
for(i in 1:m) {
  for(j in 1:n) {
    if(y[i,]==1){
      testYpredictions1[i] = testYpredictions1[i] * ( (alpha1[j])^(xtest[i,j]) * (1-alpha1[j])^(1-xtest[i,j]));
    }
  }
}
...
for(i in 1:m) {
  for(j in 1:n) {
    if(y[i,]==1){
      trainYpredictions2[i] = trainYpredictions2[i] * ( (alpha2[j])^(x[i,j]) * (1-alpha2[j])^(1-x[i,j]));
    }
  }
}
...
for(i in 1:m) {
  for(j in 1:n) {
    if(y[i,]==2){
      testYpredictions2[i] = testYpredictions2[i] * (  (alpha2[j])^(xtest[i,j]) * (1-alpha2[j])^(1-xtest[i,j]));
    }
  }
}
